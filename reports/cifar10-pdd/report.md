...
Across two seeds at 1 NFE, midpoint PDD reduced mean KID × 1,000 from **409.06** for naive Euler to **275.32**. Paired trajectory MSE to the fine teacher dropped from **0.6265** to **0.0935**. KID is noisy with 1,024 images, but the large paired-error change makes the direction unambiguous.

![Paired trajectory error](images/trajectory_error.png)

At 8 NFE, PDD retained better mean KID (**108.49** versus **176.71**) while naive Euler had lower endpoint MSE (**0.0474** versus **0.0811**). This difference is informative: PDD's samples match real-image features better, but the coarse numerical solver more closely follows the chosen fine teacher trajectory at the largest tested budget.

### 2. Midpoint targets outperform Euler

The paper reports 1-NFE ImageNet FID of 2.69 for midpoint targets versus 2.73 for Euler. In the matched grid-8 reconstruction, midpoint reduced mean 1-NFE KID from **469.31** to **275.32** and trajectory MSE from **0.1223** to **0.0935**. Midpoint was better at every tested NFE.
...
Each midpoint checkpoint used block sizes 8, 4, 2, and 1 to produce 1, 2, 4, and 8 NFE samples. Mean KID improved monotonically from **275.32** to **108.49**, while feature diversity rose from **0.115** to **0.272** toward the fine reference's **0.361**. No retraining or runtime head selection was needed.
...
|---|---|---|---|
| PDD beats naive few-step sampling | Strong large-model FID/quality | Mean 1-NFE KID 275.32 vs 409.06; MSE 0.0935 vs 0.6265 | Aligned |
| Midpoint beats Euler | 2.69 vs 2.73 ImageNet FID | Mean 275.32 vs 469.31 KID × 1,000 | Aligned |
| One model spans block sizes | Improving quality over 1/2/4/8 NFE | Mean KID 275.32 → 108.49; diversity 0.115 → 0.272 | Aligned |
